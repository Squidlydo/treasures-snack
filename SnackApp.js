...
// Distinct pin colors per retailer, so overlapping/nearby pins on the
// "All merchants" map are still tellable apart at a glance. Not brand
// colors (a couple of these retailers share a brand red) - just seven
        ...
        <>
          {Platform.OS === 'web' ? (
            <Text style={[styles.mutedSmall, { textAlign: 'center', paddingHorizontal: 16, marginBottom: 8 }]}>
              ...
              preview does not fully support this map library and will throw an error - please check
              this tab on your phone (My Device / QR code), not on the snack.expo.dev website.
            </Text>
          ) : null}
              ...
              >
                {storesToShow.map((s) => (
                  <Marker
                    ...
                    pinColor={PIN_COLORS[s.retailer] || COLORS.aqua500}
            ...
            <View style={styles.legendWrap}>
              {storesToShow.map((s) => (
                <View key={s.id} style={styles.legendItem}>
                  <View style={[styles.legendDot, { backgroundColor: PIN_COLORS[s.retailer] || COLORS.aqua500 }]} />
                  <Text style={styles.legendText}>{s.retailer}</Text>
                </View>
              ))}
            </View>
          ) : null}
...
const styles = StyleSheet.create({
  ...
  legendWrap: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 16, marginTop: 8, gap: 10 },
  legendItem: { flexDirection: 'row', alignItems: 'center', gap: 5 },
  legendDot: { width: 9, height: 9, borderRadius: 5 },
  legendText: { color: 'rgba(232,222,198,0.7)', fontSize: 10 },
